/*JavaScript Practice: Type Coercion & Explicit Conversion
===========================================================
*/


/*Problem 1 — UI Price and Quantity
A product page returns the following values as strings:
*/
let price = "500";
let quantity = "3";
let totalPrice = price * quantity;
console.log(totalPrice);
console.log(typeof totalPrice);

/*
Problem 2 — UI Price + Tax
The UI returns:
*/
let price1 = "1000";
let tax = "200";
let price1Upd = Number(price1);
let taxUpd = Number(tax);
let finalPrice = price1Upd + taxUpd;
console.log(finalPrice);
console.log(typeof finalPrice);


/*
Problem 3 — Understand + Coercion
Consider:
let actual = "100";
let expected = 20;
What will be the output of:
console.log(actual + expected);
*/
let actual = "100";
let expected = 20;
console.log(actual + expected);//10020

/*

Problem 4 — Understand - Coercion
Consider:
let actual = "100";
let expected = 20;
What will be the output of:
console.log(actual - expected); 
*/

let actual1 = "100";
let expected1 = 20;
console.log(actual1 - expected1);//80 

/*
Problem 5 — UI Value and Assertion
A UI displays:
100
The automation code receives it as:
let actualPrice = "100";
let expectedPrice = 100;
Check the result of:
console.log(actualPrice == expectedPrice);
console.log(actualPrice === expectedPrice);
*/
let actualPrice = "100";
let expectedPrice = 100;
console.log(actualPrice == expectedPrice); //true
console.log(actualPrice === expectedPrice);//false


/*
Problem 6 — Fix the Assertion
The following automation code is failing:
let actualPrice = "500";
let expectedPrice = 500;
console.log(actualPrice === expectedPrice);

Modify the code so that the assertion gives:
true
*/
console.log("-------------------------");
let actualPrice2 = "500";
let expectedPrice2 = 500;
let expectedPrice3 = String(expectedPrice2);
console.log(expectedPrice3);
console.log(typeof expectedPrice3);
console.log(actualPrice2 === expectedPrice3); //true

/*
Problem 7 — Environment Variable
An automation framework reads the timeout from an environment variable:
let timeout = "30000";
Convert it into a number and print:
The value
Its data type

Expected Output:
30000
number
*/
let timeout = "30000";
let timeoutUpd = Number(timeout);
console.log(timeoutUpd);
console.log(typeof timeoutUpd);


/*
Problem 8 — Order ID
An API returns:
let orderId = 12345;
Convert the order ID into a string and create:
ORDER-12345
*/
let orderId = 12345;
let orderIdUpd = String(orderId);
console.log("ORDER-"+orderIdUpd);

/*
Problem 9 — Boolean API Value
An API returns:
let status = "true";
Convert this value into an actual Boolean.

Expected Output:
true
boolean
*/
let status = "true";
let statusUpd = Boolean(status);
console.log(statusUpd);
console.log(typeof statusUpd);


/*
Problem 10 — Input Field Value
A Playwright test retrieves a quantity from an input field:
let quantity = "5";
The expected quantity is:
let expectedQuantity = 5;
*/
console.log("***********************");
let quantity1 = "5";
let expectedQuantity = Number(quantity1);
let expectedQuantity2 = expectedQuantity;
console.log(expectedQuantity2);
console.log(typeof expectedQuantity2);



/*
Problem 11 — Calculate Cart Total
An e-commerce application provides:
let price = "799";
let quantity = "2";
let discount = "100";
Calculate:
(price × quantity) - discount

Expected Output:
1498
*/

let cartPrice = "799";
let cartQuantity = "2";
let discount = "100";
let cartPriceUpd = Number(cartPrice);
let cartQuantityUpd = Number(cartQuantity);
let discountUpd = Number(discount);
let totalAmount =  (cartPriceUpd * cartQuantityUpd);
let finalDiscountAmount = totalAmount - discountUpd;
console.log(finalDiscountAmount);